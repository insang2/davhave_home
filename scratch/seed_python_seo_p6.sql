-- Full Python SEO Extended Lessons Part 6
...
<p><strong>Q. 파이썬 기본 리스트 대신 왜 NumPy 배열을 사용해야 하나요?</strong>
A. 파이썬 리스트는 객체 참조 주소를 연속으로 연결한 형태라 메모리가 분산되고 파이썬 인터프리터 오버헤드가 크지만, NumPy <code>ndarray</code>는 C 언어의 연속된 단일 메모리 블록에 데이터를 배치하여 처리 속도가 <strong>10~100배 가량 빠르기</strong> 때문입니다.</p>
',
  'published',
...
<ul>
<li><strong>Django</strong>: ORM, 인증 시스템, 관리자 페이지(Admin), 데이터베이스 마이그레이션이 포함된 &quot;Batteries-Included&quot; 파이썬 대표 풀스택 웹 프레임워크입니다. (MVT: Model-View-Template 패턴)</li>
...
<li><strong>ASGI (Asynchronous Server Gateway Interface)</strong>: 비동기 웹 애플리케이션과 웹 서버 간의 소통을 돕는 현대적 서버 인터페이스 규격입니다. (Uvicorn 등)</li>
<li><strong>OpenAPI (Swagger)</strong>: FastAPI에서 파이썬 코드 실행 시 엔드포인트 테스트 문서(<code>/docs</code>)를 자동 생성해 주는 인터페이스 명세 기술입니다.</li>
</ul>
<hr>
...
<hr>
<h2>3. 자주 묻는 질문 (Q&amp;A)</h2>
<p><strong>Q. Django와 FastAPI 중 어떤 것을 선택해야 하나요?</strong>
A. 관리자 웹 화면, 복잡한 사용자 로그인 인증, 템플릿 렌더링이 필요한 전통적 풀스택 웹 사이트는 <strong>Django</strong>를 선호하고, 마이크로서비스(MSA), 인공지능 API 서버, 모바일 앱 백엔드 REST API는 <strong>FastAPI</strong>를 선택하는 것이 실무 표준입니다.</p>
',
  'published',
...
A. 데이터베이스 종류(MySQL $ightarrow$ PostgreSQL)가 변경되어도 SQL 문법을 고칠 필요가 없으며, SQL Injection 해킹 공격을 방지하고 객체지향적인 깔끔한 코드를 유지할 수 있습니다.
',
...
<hr>
<h2>1. 파이썬 ORM 용어 사전 (Glossary)</h2>
<ul>
<li><strong>ORM (Object-Relational Mapping)</strong>: 파이썬의 클래스/객체와 RDBMS(MySQL, PostgreSQL, SQLite)의 데이터 테이블을 자동으로 1:1 연결해 주는 매핑 기술입니다.</li>
<li><strong>SQLAlchemy</strong>: 파이썬 최고 수준의 유연성과 안정성을 자랑하는 대표적 엔터프라이즈 ORM 라이브러리입니다.</li>
<li><strong>Engine / Session</strong>: 데이터베이스 연결 커넥션 풀을 관리하는 <code>Engine</code>과 DB 트랜잭션 작업을 단위별로 관리하는 <code>Session</code> 객체입니다.</li>
<li><strong>Alembic</strong>: SQLAlchemy DB 모델의 구조 변경 이력을 데이터베이스에 안전하게 반영하고 되돌리는 마이그레이션(Migration) 도구입니다.</li>
</ul>
<hr>
<h2>2. SQLAlchemy 2.0 기반 DB CRUD 코드</h2>
<pre><code class="language-python">from sqlalchemy import create_engine, String
from sqlalchemy.orm import DeclarativeBase, Mapped, mapped_column, Session
...
<hr>
<h2>3. 자주 묻는 질문 (Q&amp;A)</h2>
<p><strong>Q. 생 SQL(Raw SQL)보다 ORM을 사용하면 어떤 이점이 있나요?</strong>
A. 데이터베이스 종류(MySQL $
ightarrow$ PostgreSQL)가 변경되어도 SQL 문법을 고칠 필요가 없으며, SQL Injection 해킹 공격을 방지하고 객체지향적인 깔끔한 코드를 유지할 수 있습니다.</p>
',
  'published',
...
<hr>
<h2>1. 웹 자동화 용어 사전 (Glossary)</h2>
<ul>
<li><strong>Headless Browser</strong>: 디스플레이 모니터 화면(GUI)을 띄우지 않고 메모리 상에서 백그라운드로 고속 구동되는 브라우저 엔진입니다.</li>
<li><strong>Selenium</strong>: 가장 오랜 역사를 지닌 웹 브라우저 자동화 및 테스트 표준 툴킷입니다.</li>
<li><strong>Playwright</strong>: Microsoft에서 개발한 차세대 비동기 웹 자동화 도구로, 기존 Selenium 대비 5배 이상 빠르고 브라우저 컨텍스트 격리를 지원합니다.</li>
<li><strong>WebDriver</strong>: 자동화 스크립트가 크롬/파이어폭스 브라우저에 이벤트를 전달할 수 있게 돕는 드라이버 매니저입니다.</li>
</ul>
<hr>
<h2>2. Playwright 비동기 브라우저 자동화 코드</h2>
...
<hr>
<h2>3. 자주 묻는 질문 (Q&amp;A)</h2>
<p><strong>Q. <code>requests</code>로 크롤링할 때와 <code>Selenium/Playwright</code>를 쓸 때의 차이는?</strong>
A. <code>requests</code>는 순수 HTML 텍스트만 긁어오므로 자바스크립트가 나중에 데이터를 렌더링하는 SPA(Single Page Application) 페이지는 데이터를 가져올 수 없습니다. 반면 <code>Playwright</code>는 실제 브라우저가 자바스크립트를 완전히 실행한 후의 최종 화면 데이터를 가져옵니다.</p>
',
  'published',
