...
**Q. 카티션 곱과 자연 조인의 차이는 무엇인가요?**
A. 카티션 곱($R \times S$)은 아무런 연관 조건 없이 두 테이블의 행들을 모든 경우의 수로 조합하여 튜플 수가 폭증하는 반면, 자연 조인($R \bowtie S$)은 같은 이름의 공통 속성 값이 일치하는 행들만 결합하여 중복 컬럼을 제거한 깔끔한 결과셋을 만듭니다.
`
  },
  // 6
  {
...
| **제3정규형 (3NF)** | 2NF를 만족하고, **이행적 함수 종속성(Transitive FD)**을 제거해야 함 | $X \rightarrow Y \rightarrow Z$ 형태의 간접 종속성 분리 |
...
chunks.forEach((chunk, index) => {
  let sqlContent = `-- Ultimate Database Mega Lessons Part ${index + 1}\n`;
...
`;
  ...
  fs.writeFileSync(`scratch/seed_database_ultimate_p${index + 1}.sql`, sqlContent, 'utf-8');
});

console.log('Successfully generated 5 Ultimate Database SQL chunk files!');
