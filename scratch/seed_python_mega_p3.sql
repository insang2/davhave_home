-- Full Python Mega Lessons Part 3
...
<ul>
<li><strong>Iterator</strong>: <code>__next__()</code> 메소드를 호출할 때마다 다음 요소를 하나씩 꺼내주는 순회 객체입니다.</li>
...
<hr>
<h2>3. 자주 묻는 질문 (Q&amp;A)</h2>
<p><strong>Q. 왜 큰 리스트 대신 제너레이터를 써야 하나요?</strong>
...
<ul>
<li><strong>datetime / timedelta</strong>: 날짜와 시간을 생성하고 날짜 간의 차이를 계산하는 시간 처리 모듈입니다.</li>
<li><strong>json (JSON Serialization)</strong>: 파이썬 객체를 JSON 텍스트로 변환(<code>dumps()</code>)하거나 파싱(<code>loads()</code>)하는 직렬화 모듈입니다.</li>
<li><strong>re (Regular Expression)</strong>: 텍스트 내에서 특정 규칙을 가진 문자열 패턴을 검색/추출/치환하는 정규 표현식 모듈입니다.</li>
</ul>
<hr>
...
A. <code>dumps()</code>는 파이썬 객체를 JSON 형식의 **문자열(String)**로 반환하고, <code>dump()</code>는 바로 **파일 객체(File Stream)**에 JSON 데이터를 씁니다.</p>
',
  'published',
