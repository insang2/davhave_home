-- Full Python Mega Lessons Part 2
...
A. <code>close()</code>를 하지 않으면 메모리 버퍼에 남아있는 데이터가 디스크에 즉시 기록되지 않거나, 파일 락 자원이 유지되어 다른 프로세스가 파일에 접근할 수 없게 됩니다. <code>with</code> 문을 쓰면 자동 마감됩니다.</p>
',
  'published',
