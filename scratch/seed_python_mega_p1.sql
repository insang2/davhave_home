-- Full Python Mega Lessons Part 1
...
',
  'published',
  '파이썬 공식 가이드 - CPython 인터프리터, Zen of Python, IDLE & VS Code',
...
<hr>
<h2>2. match-case 패턴 매칭 &amp; zip 실습</h2>
<pre><code class="language-python"># 1. 파이썬 3.10+ match-case 구조적 패턴 매칭
def process_command(command):
    match command.split():
        case [&quot;quit&quot;]:
            print(&quot;프로그램을 종료합니다.&quot;)
...
A. for 루프가 <code>break</code> 문을 만나 중단되지 않고 <strong>정상적으로 처음부터 끝까지 모두 완주했을 때</strong> 마지막에 <code>else</code> 블록이 1회 실행됩니다.</p>
',
  'published',
