-- Ultimate Database Mega Lessons Part 2
...
A. 카티션 곱($R 	imes S$)은 아무런 연관 조건 없이 두 테이블의 행들을 모든 경우의 수로 조합하여 튜플 수가 폭증하는 반면, 자연 조인($R owtie S$)은 같은 이름의 공통 속성 값이 일치하는 행들만 결합하여 중복 컬럼을 제거한 깔끔한 결과셋을 만듭니다.
',
...
<p><strong>Q. 카티션 곱과 자연 조인의 차이는 무엇인가요?</strong>
A. 카티션 곱($R 	imes S$)은 아무런 연관 조건 없이 두 테이블의 행들을 모든 경우의 수로 조합하여 튜플 수가 폭증하는 반면, 자연 조인($R owtie S$)은 같은 이름의 공통 속성 값이 일치하는 행들만 결합하여 중복 컬럼을 제거한 깔끔한 결과셋을 만듭니다.</p>
',
  'published',
