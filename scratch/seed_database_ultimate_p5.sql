-- Ultimate Database Mega Lessons Part 5
...
1. **Atomicity (원자성)**: 트랜잭션 내의 모든 연산은 "All or Nothing"으로 완벽히 전부 수행되거나 아예 실행되지 않아야 합니다. (DBMS 회복 모듈)
2. **Consistency (일관성)**: 트랜잭션 실행 전후의 데이터베이스는 항상 정해진 무결성 제약조건을 만족해야 합니다.
3. **Isolation (격리성)**: 실행 중인 트랜잭션에 다른 동시 트랜잭션이 끼어들어 중간 상태를 참조할 수 없습니다. (동시성 제어 모듈)
4. **Durability (지속성)**: 성공적으로 완료(COMMIT)된 트랜잭션의 결과는 시스템 장애가 발생해도 메모리 및 디스크에 영구 보존되어야 합니다. (로그 복구)
...
-- 송금 트랜잭션 예시
-- 1. A 계좌 잔액 차감
UPDATE account SET balance = balance - 10000 WHERE acc_no = ''A101'';

-- 2. 중간 저장점 선언
SAVEPOINT sp1;

-- 3. B 계좌 잔액 가산
UPDATE account SET balance = balance + 10000 WHERE acc_no = ''B202'';

-- 4. 문제 발생 시 저장점으로 복구 또는 전체 확정
COMMIT; -- 영구 저장
```
',
...
<ol>
<li><strong>Atomicity (원자성)</strong>: 트랜잭션 내의 모든 연산은 &quot;All or Nothing&quot;으로 완벽히 전부 수행되거나 아예 실행되지 않아야 합니다. (DBMS 회복 모듈)</li>
<li><strong>Consistency (일관성)</strong>: 트랜잭션 실행 전후의 데이터베이스는 항상 정해진 무결성 제약조건을 만족해야 합니다.</li>
<li><strong>Isolation (격리성)</strong>: 실행 중인 트랜잭션에 다른 동시 트랜잭션이 끼어들어 중간 상태를 참조할 수 없습니다. (동시성 제어 모듈)</li>
<li><strong>Durability (지속성)</strong>: 성공적으로 완료(COMMIT)된 트랜잭션의 결과는 시스템 장애가 발생해도 메모리 및 디스크에 영구 보존되어야 합니다. (로그 복구)</li>
</ol>
<hr>
<h2>2. 트랜잭션 제어 구문 실습</h2>
<pre><code class="language-sql">-- 송금 트랜잭션 예시
-- 1. A 계좌 잔액 차감
UPDATE account SET balance = balance - 10000 WHERE acc_no = &#39;A101&#39;;
...
- **Non-Repeatable Read**: 한 트랜잭션 내에서 동일한 데이터를 두 번 읽을 때 중간에 다른 트랜잭션이 해당 데이터를 UPDATE하여 값이 다르게 나타나는 현상입니다.
- **Phantom Read**: 한 트랜잭션 내에서 범위 쿼리를 두 번 실행할 때 중간에 다른 트랜잭션이 INSERT하여 이전에 없던 새로운 유령 행이 나타나는 현상입니다.
...
| **SERIALIZABLE** | 방지 | 방지 | 방지 (완전 직렬화 실행) |
',
...
<li><strong>Non-Repeatable Read</strong>: 한 트랜잭션 내에서 동일한 데이터를 두 번 읽을 때 중간에 다른 트랜잭션이 해당 데이터를 UPDATE하여 값이 다르게 나타나는 현상입니다.</li>
<li><strong>Phantom Read</strong>: 한 트랜잭션 내에서 범위 쿼리를 두 번 실행할 때 중간에 다른 트랜잭션이 INSERT하여 이전에 없던 새로운 유령 행이 나타나는 현상입니다.</li>
</ul>
<hr>
...
<thead>
<tr>
<th align="left">격리 수준 (Isolation Level)</th>
...
<td align="center">방지</td>
<td align="center">방지</td>
<td align="center"><strong>발생</strong> (MySQL InnoDB는 MVCC로 방지)</td>
</tr>
<tr>
<td align="left"><strong>SERIALIZABLE</strong></td>
<td align="center">방지</td>
<td align="center">방지</td>
<td align="center">방지 (완전 직렬화 실행)</td>
</tr>
</tbody></table>
',
  'published',
...
- **BASE**: NoSQL이 일관성을 조금 희생하고 가용성과 확장성을 얻기 위해 따르는 원칙입니다. (**B**asically **A**vailable, **S**oft-state, **E**ventual consistency)
- **NewSQL**: ACID 트랜잭션 속성을 완벽히 보장하면서도 NoSQL의 초고속 수평 분산 확장 능력을 결합한 차세대 DB입니다. (예: CockroachDB, Google Spanner)
...
| **Wide-Column Store** | **Cassandra**, HBase | 행마다 다른 컬럼을 가질 수 있는 열 중심 분산 저장 (빅데이터 타임시리즈) |
...
<li><strong>BASE</strong>: NoSQL이 일관성을 조금 희생하고 가용성과 확장성을 얻기 위해 따르는 원칙입니다. (<strong>B</strong>asically <strong>A</strong>vailable, <strong>S</strong>oft-state, <strong>E</strong>ventual consistency)</li>
<li><strong>NewSQL</strong>: ACID 트랜잭션 속성을 완벽히 보장하면서도 NoSQL의 초고속 수평 분산 확장 능력을 결합한 차세대 DB입니다. (예: CockroachDB, Google Spanner)</li>
</ul>
<hr>
...
<td align="left">행마다 다른 컬럼을 가질 수 있는 열 중심 분산 저장 (빅데이터 타임시리즈)</td>
</tr>
<tr>
