-- Fix duplicated H1 title in D1 DB
...
',
  content_html = '<p>**스택(Stack)**은 한쪽 끝에서만 데이터의 삽입과 삭제가 일어나는 선형 자료구조입니다. 가장 나중에 들어간 데이터가 가장 먼저 나오는 <strong>후입선출(LIFO, Last-In First-Out)</strong> 메커니즘을 따릅니다.</p>
<hr>
...
WHERE kind = 'education' AND category = 'algorithm' AND slug = 'ch01-stack';
...
',
  content_html = '<p>**큐(Queue)**는 데이터의 삽입과 삭제가 서로 다른 끝에서 일어나는 선형 자료구조입니다. 먼저 들어온 데이터가 먼저 나가는 <strong>선입선출(FIFO, First-In First-Out)</strong> 구조를 가집니다.</p>
<hr>
...
',
  updated_at = CURRENT_TIMESTAMP
WHERE kind = 'education' AND category = 'algorithm' AND slug = 'ch02-queue';
...
',
  content_html = '<p>**연결 리스트(Linked List)**는 각 원소가 데이터와 다음 원소를 가리키는 주소(포인터)를 포함하는 **노드(Node)**들로 구성된 동적 선형 자료구조입니다.</p>
<hr>
...
',
  updated_at = CURRENT_TIMESTAMP
WHERE kind = 'education' AND category = 'algorithm' AND slug = 'ch03-linked-list';
...
',
  content_html = '<p>**트리(Tree)**는 계층적인 관계를 나타내는 대표적인 비선형(Non-linear) 자료구조입니다. 하나의 뿌리(Root) 노드에서 시작하여 나뭇가지처럼 늘어나는 형태를 취합니다.</p>
<hr>
<h2>1. 트리의 용어 및 구조</h2>
<ul>
...
',
  updated_at = CURRENT_TIMESTAMP
WHERE kind = 'education' AND category = 'algorithm' AND slug = 'ch04-tree';
...
',
  content_html = '<p>**알고리즘(Algorithm)**이란 명확히 정의된 문제를 해결하거나 특정 입력을 출력으로 변환하기 위한 단계적인 계산 절차를 의미합니다.</p>
<hr>
...
',
  updated_at = CURRENT_TIMESTAMP
WHERE kind = 'education' AND category = 'algorithm' AND slug = 'ch05-algorithm-analysis';
...
',
  content_html = '<p>**정렬(Sorting)**은 순서가 없는 데이터 집합을 특정 키(Key) 값의 순서대로 재배열하는 작업입니다.</p>
<hr>
...
',
  updated_at = CURRENT_TIMESTAMP
WHERE kind = 'education' AND category = 'algorithm' AND slug = 'ch06-sorting-algorithms';
...
',
  content_html = '<p>**탐색(Searching)**은 데이터 구조에 저장된 수많은 값 중에서 원하는 키(Key)를 가진 항목을 찾아내는 프로세스입니다.</p>
<hr>
<h2>1. 탐색 알고리즘 비교</h2>
<table>
<thead>
...
',
  updated_at = CURRENT_TIMESTAMP
WHERE kind = 'education' AND category = 'algorithm' AND slug = 'ch07-searching-algorithms';
...
',
  content_html = '<p>**그래프(Graph)**는 정점(Vertex)들과 이들을 연결하는 간선(Edge)들의 집합 $G = (V, E)$로 표현되는 비선형 자료구조입니다.</p>
<hr>
<h2>1. 그래프 표현 방식</h2>
<ol>
...
',
  updated_at = CURRENT_TIMESTAMP
WHERE kind = 'education' AND category = 'algorithm' AND slug = 'ch08-graph-algorithms';
...
',
  content_html = '<p>문제 해결 전략 중 **완전 탐색(Brute-Force)**과 **탐욕 알고리즘(Greedy Algorithm)**은 가장 대표적이고 기본적인 설계 패러다임입니다.</p>
<hr>
...
',
  updated_at = CURRENT_TIMESTAMP
WHERE kind = 'education' AND category = 'algorithm' AND slug = 'ch09-brute-force-and-greedy';
...
',
  content_html = '<p>**분할 정복(Divide and Conquer)**은 복잡하고 큰 문제를 해결 가능한 작은 문제들로 분할하여 각각 해결(정복)한 후, 그 결과들을 결합하는 알고리즘 설계 패러다임입니다.</p>
<hr>
...
',
  updated_at = CURRENT_TIMESTAMP
WHERE kind = 'education' AND category = 'algorithm' AND slug = 'ch10-divide-and-conquer';
...
',
  content_html = '<p>**동적 계획법(Dynamic Programming, DP)**은 복잡한 문제를 작은 부분 문제들로 나눈 후, 그 부분 문제의 해를 메모리에 저장하여 동일한 계산의 중복을 방지하는 최적화 기법입니다.</p>
<hr>
...
',
  updated_at = CURRENT_TIMESTAMP
WHERE kind = 'education' AND category = 'algorithm' AND slug = 'ch11-dynamic-programming';
...
',
  content_html = '<p>알고리즘 디자인에서는 <strong>메모리(공간)를 더 써서 시간(속도)을 버는 트레이드오프 기법</strong>과, 가능성 없는 경로를 빠르게 포기하는 <strong>백트래킹(Backtracking)</strong> 기법이 매우 중요하게 다루어집니다.</p>
<hr>
...
',
  updated_at = CURRENT_TIMESTAMP
WHERE kind = 'education' AND category = 'algorithm' AND slug = 'ch12-space-time-tradeoff-and-backtracking';
