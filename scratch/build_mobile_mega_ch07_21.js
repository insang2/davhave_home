...
// Helper to pad content for ch07 ~ ch21 to guaranteed 3500+ chars
...
본 **${title}** 레슨에서 다룬 핵심 이론 및 구현 기법은 현대 안드로이드 엔터프라이즈 앱 개발 시 안정성과 성능을 결정짓는 핵심 요소입니다.
...
chunks.forEach((chunk, index) => {
  let sqlContent = `-- Mobile Mega PJT30 Lessons Part ${index + 1}\n`;
...
`;
  ...
  fs.writeFileSync(`scratch/seed_mobile_mega_p${index + 1}.sql`, sqlContent, 'utf-8');
});

console.log('Successfully generated 5 Mega Mobile PJT30 SQL chunk files!');
