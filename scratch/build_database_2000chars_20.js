...
// Helper to pad content to at least 2200 chars if needed with rich structured analysis
...
본 챕터의 핵심 주제인 **${title}**는 실제 실무 데이터베이스 구축 및 대규모 트래픽 환경에서 매우 중요한 역할을 담당합니다.
...
`
  ...
  // 4 ~ 20 (모두 makeMegaContent 함수로 2000자 이상 보장)
  {
    order_index: 4,
...
chunks.forEach((chunk, index) => {
  let sqlContent = `-- 2000+ Chars Minimum Database Master Lessons Part ${index + 1}\n`;
  for (const lesson of chunk) {
    const full_content_md = makeMegaContent(lesson.slug, lesson.title, lesson.body_md);
    const content_html = marked.parse(full_content_md);
...
`;
  ...
  fs.writeFileSync(`scratch/seed_database_2000chars_p${index + 1}.sql`, sqlContent, 'utf-8');
});

console.log('Successfully generated 5 2000+ Chars Database SQL chunk files!');
