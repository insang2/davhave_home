...
const lessons = [
  {
    order_index: 1,
    slug: 'ch01-stack',
    title: '스택(Stack)의 개념과 구현: LIFO 원리와 활용',
    seo_title: '스택(Stack) 자료구조 완벽 정리 - LIFO 원리, 연산 및 파이썬 구현',
    seo_description: '스택(Stack) 자료구조의 후입선출(LIFO) 원리, push/pop/peek 연산, 함수 호출 스택, 괄호 검사 알고리즘과 코드 구현법을 상세히 다룹니다.',
    excerpt: '후입선출(LIFO) 방식을 따르는 선형 자료구조인 스택의 핵심 개념, 주요 연산(push, pop, peek), 함수 호출 스택 및 수식 괄호 검사 알고리즘을 학습합니다.',
    content_md: `**스택(Stack)**은 한쪽 끝에서만 데이터의 삽입과 삭제가 일어나는 선형 자료구조입니다. 가장 나중에 들어간 데이터가 가장 먼저 나오는 **후입선출(LIFO, Last-In First-Out)** 메커니즘을 따릅니다.
...
`
  },
  {
    order_index: 2,
    slug: 'ch02-queue',
    title: '큐(Queue) 자료구조: FIFO 원리, 원형 큐, 우선순위 큐',
    seo_title: '큐(Queue) 자료구조 총정리 - FIFO 원리, 원형 큐, Deque, 우선순위 큐',
    seo_description: '선입선출(FIFO) 기반 큐의 동작 메커니즘, 선형 큐의 오버플로 문제 극복을 위한 원형 큐(Circular Queue), Deque, 우선순위 큐(Priority Queue) 개념을 정리합니다.',
    excerpt: '가장 먼저 들어온 데이터가 먼저 나가는 FIFO 구조인 큐(Queue)의 기초, 선형 큐의 한계를 극복하는 원형 큐, Deque 및 우선순위 큐의 구조를 상세히 다룹니다.',
    content_md: `**큐(Queue)**는 데이터의 삽입과 삭제가 서로 다른 끝에서 일어나는 선형 자료구조입니다. 먼저 들어온 데이터가 먼저 나가는 **선입선출(FIFO, First-In First-Out)** 구조를 가집니다.
...
`
  ...
  {
    order_index: 3,
    slug: 'ch03-linked-list',
    title: '연결 리스트(Linked List): 단일, 이중, 원형 연결 리스트',
    seo_title: '연결 리스트(Linked List) 개념과 구현 - 단일·이중·원형 연결 리스트 비교',
    seo_description: '동적 메모리 할당 기반의 연결 리스트 구조, 배열(Array)과의 성능 비교, 단일/이중/원형 연결 리스트의 노드 관리 및 파이썬 구현법을 설명합니다.',
    excerpt: '노드(Node)와 포인터(Pointer)의 연결로 구성되는 동적 자료구조인 연결 리스트의 원리, 배열과의 차이점, 단일/이중/원형 연결 리스트의 특징을 학습합니다.',
    content_md: `**연결 리스트(Linked List)**는 각 원소가 데이터와 다음 원소를 가리키는 주소(포인터)를 포함하는 **노드(Node)**들로 구성된 동적 선형 자료구조입니다.
...
`
  },
  {
    order_index: 4,
    slug: 'ch04-tree',
    title: '트리(Tree) 자료구조: 이진 트리와 순회 알고리즘',
    seo_title: '트리(Tree) 자료구조 완전 분석 - 이진 트리, 전위/중위/후위 순회, 수식 트리',
    seo_description: '계층적 비선형 구조인 트리의 용어(노드, 간선, 차수, 높이), 이진 트리(Binary Tree) 종류와 전위·중위·후위 순회 알고리즘, 수식 트리 응용을 다룹니다.',
    excerpt: '계층적 데이터 표현을 위한 비선형 자료구조인 트리의 기본 개념, 이진 트리(Binary Tree), 전위/중위/후위 순회 및 수식 트리 응용법을 다룹니다.',
    content_md: `**트리(Tree)**는 계층적인 관계를 나타내는 대표적인 비선형(Non-linear) 자료구조입니다. 하나의 뿌리(Root) 노드에서 시작하여 나뭇가지처럼 늘어나는 형태를 취합니다.
...
`
  ...
  {
    order_index: 5,
    slug: 'ch05-algorithm-analysis',
    title: '알고리즘 개요와 복잡도 분석: Big-O 표기법',
    seo_title: '알고리즘 복잡도 분석 guide - Big-O 표기법, 시간 복잡도, 공간 복잡도',
    seo_description: '알고리즘의 성립 조건, 점근적 분석법(Big-O, Big-Omega, Big-Theta), 최악/평균/최선 시간복잡도와 공간복잡도 계산법을 체계적으로 안내합니다.',
    excerpt: '주어진 문제를 해결하기 위한 단계적 절차인 알고리즘의 정의, 5대 조건, Big-O 표기법을 통한 시간 및 공간 복잡도 분석 기법을 상세히 다룹니다.',
    content_md: `**알고리즘(Algorithm)**이란 명확히 정의된 문제를 해결하거나 특정 입력을 출력으로 변환하기 위한 단계적인 계산 절차를 의미합니다.
...
`
  ...
  {
    order_index: 6,
    slug: 'ch06-sorting-algorithms',
    title: '정렬 알고리즘(Sorting): 선택, 삽입, 퀵, 병합, 기수 정렬',
    seo_title: '대표 정렬 알고리즘 비교 분석 - O(N^2) vs O(N log N), 정렬 안정성',
    seo_description: '선택·삽입·버블 정렬부터 퀵 정렬, 병합 정렬, 기수 정렬까지 각 정렬 알고리즘의 원리, 시간복잡도, 정렬 안정성(Stability)을 비교합니다.',
    excerpt: '데이터를 일정한 순서(오름차순/내림차순)로 재배열하는 주요 정렬 알고리즘의 동작 원리, 시간복잡도 비교 및 정렬 안정성을 체계적으로 살펴봅니다.',
    content_md: `**정렬(Sorting)**은 순서가 없는 데이터 집합을 특정 키(Key) 값의 순서대로 재배열하는 작업입니다.
...
`
  ...
  {
    order_index: 7,
    slug: 'ch07-searching-algorithms',
    title: '탐색 알고리즘(Searching): 순차 탐색, 이진 탐색, BST',
    seo_title: '탐색 알고리즘 총정리 - 순차 탐색, 이진 탐색, 이진 탐색 트리(BST) 연산',
    seo_description: '선형 탐색과 이진 탐색의 비교, 정렬된 배열에서의 O(log N) 이진 탐색 원리, 이진 탐색 트리(BST)의 탐색·삽입·삭제 동작을 해설합니다.',
    excerpt: '대량의 데이터 집합에서 원하는 조건의 키(Key) 값을 찾는 순차 탐색, 이진 탐색 및 이진 탐색 트리(BST) 알고리즘을 체계적으로 다룹니다.',
    content_md: `**탐색(Searching)**은 데이터 구조에 저장된 수많은 값 중에서 원하는 키(Key)를 가진 항목을 찾아내는 프로세스입니다.
...
`
  ...
  {
    order_index: 8,
    slug: 'ch08-graph-algorithms',
    title: '그래프(Graph) 알고리즘: DFS, BFS, MST, 최단 경로',
    seo_title: '그래프(Graph) 알고리즘 핵심 가이드 - DFS, BFS, Prim, Kruskal, Dijkstra',
    seo_description: '그래프의 표현(인접 행렬 vs 인접 리스트), 깊이 우선 탐색(DFS)과 너비 우선 탐색(BFS), 최소 신장 트리(MST), 다익스트라(Dijkstra) 최단 경로 알고리즘을 다룹니다.',
    excerpt: '정점(Vertex)과 간선(Edge)으로 구성된 복잡한 객체 간의 관계를 다루는 그래프 구조, DFS/BFS 순회, 최소 신장 트리(MST) 및 다익스트라 알고리즘을 학습합니다.',
    content_md: `**그래프(Graph)**는 정점(Vertex)들과 이들을 연결하는 간선(Edge)들의 집합 $G = (V, E)$로 표현되는 비선형 자료구조입니다.
...
`
  ...
  {
    order_index: 9,
    slug: 'ch09-brute-force-and-greedy',
    title: '완전 탐색(Brute-Force)과 탐욕 알고리즘(Greedy Strategy)',
    seo_title: '알고리즘 설계 전략 - 완전 탐색(Brute-force) 및 탐욕 알고리즘(Greedy)',
    seo_description: '모든 경우의 수를 검증하는 완전 탐색 기법과 매 순간 최적의 선택을 하는 탐욕 알고리즘의 성립 조건(탐욕적 선택 속성, 최적 부분 구조)을 분석합니다.',
    excerpt: '가능한 모든 해를 다 검증하는 완전 탐색(Brute-Force) 기법과 매 순간 최적인 선택을 내리는 탐욕 알고리즘(Greedy Strategy)의 설계 원리를 다룹니다.',
    content_md: `문제 해결 전략 중 **완전 탐색(Brute-Force)**과 **탐욕 알고리즘(Greedy Algorithm)**은 가장 대표적이고 기본적인 설계 패러다임입니다.
...
`
  ...
  {
    order_index: 10,
    slug: 'ch10-divide-and-conquer',
    title: '분할 정복(Divide and Conquer) 전략과 응용',
    seo_title: '분할 정복(Divide and Conquer) 알고리즘 - 원리, 병합 정렬, Quick Select',
    seo_description: '문제 해결의 3단계(Divide, Conquer, Combine), 거듭제곱 계산, k-번째 작은 수 찾기(Quick Select), 병합 정렬에서의 재귀적 분할 기법을 학습합니다.',
    excerpt: '큰 문제를 작은 독립적인 부분 문제로 나누어 각각 해결한 후 결과를 합성하는 분할 정복(Divide and Conquer) 전략의 원리와 응용 사례를 학습합니다.',
    content_md: `**분할 정복(Divide and Conquer)**은 복잡하고 큰 문제를 해결 가능한 작은 문제들로 분할하여 각각 해결(정복)한 후, 그 결과들을 결합하는 알고리즘 설계 패러다임입니다.
...
$$a^n = \begin{cases} (a^{n/2})^2 & \text{if } n \text{ is even} \\ a \times (a^{(n-1)/2})^2 & \text{if } n \text{ is odd} \end{cases}$$
...
`
  ...
  {
    order_index: 11,
    slug: 'ch11-dynamic-programming',
    title: '동적 계획법(Dynamic Programming, DP) 개념과 패러다임',
    seo_title: '동적 계획법(Dynamic Programming) 완벽 이해 - Memoization vs Tabulation, LCS, 배낭 문제',
    seo_description: '중복 부분 문제와 최적 부분 구조 특성을 활용하는 동적 계획법, 하향식(Memoization)과 상향식(Tabulation) 접근법, LCS 및 배낭 문제(Knapsack) 구현을 다룹니다.',
    excerpt: '중복되는 부분 문제의 답을 저장하고 재활용하여 계산 효율을 획기적으로 높이는 동적 계획법(Dynamic Programming)의 핵심 패러다임과 실전 문제를 다룹니다.',
    content_md: `**동적 계획법(Dynamic Programming, DP)**은 복잡한 문제를 작은 부분 문제들로 나눈 후, 그 부분 문제의 해를 메모리에 저장하여 동일한 계산의 중복을 방지하는 최적화 기법입니다.
...
`
  ...
  {
    order_index: 12,
    slug: 'ch12-space-time-tradeoff-and-backtracking',
    title: '공간-시간 트레이드오프와 백트래킹(Backtracking)',
    seo_title: '공간-시간 트레이드오프(Space-Time Tradeoff) & 백트래킹 알고리즘 - 해싱, N-Queen',
    seo_description: '메모리를 활용해 계산 속도를 높이는 기법(해싱, 사전 계산), 상태 공간 트리를 탐색하며 불필요한 경로를 가지치기(Pruning)하는 백트래킹 알고리즘과 N-Queen 문제를 다룹니다.',
    excerpt: '추가 메모리 공간을 투자하여 연산 속도를 얻는 공간-시간 트레이드오프 개념 및 상태 공간 트리를 효율적으로 탐색하는 백트래킹(Backtracking) 기법을 학습합니다.',
    content_md: `알고리즘 디자인에서는 **메모리(공간)를 더 써서 시간(속도)을 버는 트레이드오프 기법**과, 가능성 없는 경로를 빠르게 포기하는 **백트래킹(Backtracking)** 기법이 매우 중요하게 다루어집니다.
...
let sqlContent = `-- Fix duplicated H1 title in D1 DB\n`;
...
for (const lesson of lessons) {
  ...
  sqlContent += `
UPDATE posts SET
  content_md = '${escapeSql(lesson.content_md)}',
  content_html = '${escapeSql(content_html)}',
  updated_at = CURRENT_TIMESTAMP
WHERE kind = 'education' AND category = 'algorithm' AND slug = '${escapeSql(lesson.slug)}';
`;
}

fs.writeFileSync('scratch/fix_algorithm_titles.sql', sqlContent, 'utf-8');
console.log('Successfully generated scratch/fix_algorithm_titles.sql!');
