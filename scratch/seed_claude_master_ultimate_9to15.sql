...
def mask_private_info(text):
    ...
    return text
```
', '<p>Anthropic 공식 <strong>Claude Code &amp; Cowork 마스터 가이드 9편</strong>에서는 <strong>한국 실무 환경 특화 가이드</strong>로 HWP/HWPX 문서 파싱, 한국어 비즈니스 어조 조정, 개인정보 마스킹 및 대기업/공공 기관 보안 규정 대처법을 입문자의 눈높이에 맞춰 세밀하게 다룹니다.</p>
<hr>
<h2>1. HWP 및 스캔 PDF 문서 파싱 파이프라인</h2>
<p>한국 기업 및 공공기관 업무에서 가장 흔하게 접하는 HWP/HWPX 문서 및 PDF의 표(Table)와 텍스트를 손실 없이 마크다운 구조화 데이터로 이관하는 정밀 가이드입니다.</p>
...
<p>상황과 대상에 따라 AI 답변의 어조를 정밀하게 조율하는 프롬프트 지침입니다:</p>
<ul>
<li><strong>경영진 보고서</strong>: &quot;~함&quot;, &quot;~임&quot;, &quot;~으로 판단됨&quot; 형태의 격식체 요약</li>
<li><strong>고객 대면 메시지</strong>: 친절하고 명확한 비즈니스 경어체 (&quot;~해 드리겠습니다&quot;)</li>
<li><strong>사내 Slack 알림</strong>: 간결한 구어체 서술형 (&quot;~가 반영되었습니다&quot;)</li>
</ul>
<hr>
<h2>3. 대기업 및 공공기관 보안 규정 대처: 개인정보 마스킹 (Masking)</h2>
<p>AI에 데이터를 전달하기 전 주민등록번호, 전화번호, 이메일을 자동 마스킹하는 파이썬 정규식 수칙입니다.</p>
...
', '<p>Claude 마스터 가이드 10편에서는 <strong>커뮤니티 검증 5대 활용 패턴</strong>과 <strong>공식 vs 커뮤니티 스킬/플러그인 선별법</strong>을 초보자가 실습을 통해 따라 할 수 있도록 해설합니다.</p>
<hr>
<h2>1. 커뮤니티 검증 5대 활용 패턴</h2>
<ol>
<li><strong>Dual-Agent Review Pattern (이중 검수)</strong>: 1번 에이전트가 코드를 작성하고, 2번 보안 에이전트가 독립적으로 검수</li>
...
<li><strong>Git Branch Isolation</strong>: 모든 AI 작업은 별도 브랜치(<code>feature/ai-task</code>)에서 구동 후 안전 병합</li>
<li><strong>Incremental Memory Update</strong>: 작업 진행 현황을 <code>Task_Progress.md</code> 파일에 매 단계 1줄 갱신</li>
</ol>
<hr>
...
npx wrangler deploy
```
', '<p>Claude 마스터 가이드 12편에서는 초보자도 그대로 따라 할 수 있는 <strong>풀스택 웹사이트 구축 및 Cloudflare Edge 배포 실전 프로젝트</strong>를 단계별 소스 코드와 함께 제공합니다.</p>
<hr>
...
```
', '<p>Claude 마스터 가이드 13편에서는 초보자도 즉시 구동할 수 있는 <strong>비즈니스 보고서 자동 생성 및 Slack 웹훅 자동 발송 스크립트 프로젝트</strong>를 파이썬 풀 코드와 함께 제공합니다.</p>
<hr>
...
', '<p>Claude 마스터 가이드 14편에서는 초보자가 꼭 알아야 할 <strong>AI 전문 용어 20선</strong>과 실무에서 바로 복사해 쓰는 <strong>한 줄 프롬프트 치트시트</strong>를 수록합니다.</p>
<hr>
