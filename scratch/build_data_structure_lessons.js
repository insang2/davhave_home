...
const lessons = [
  {
    order_index: 1,
    slug: 'ch01-data-structures-and-python-basics',
    title: '자료구조 개요와 파이썬 프로그래밍 기초',
    seo_title: '자료구조 개요 및 파이썬 기초 - ADT, 객체지향, 시간복잡도 분석',
    seo_description: '자료구조의 개념(추상 자료형 ADT), 파이썬 클래스 기반 객체지향 설계, 리스트 연산과 시간복잡도, 고차함수(filter, map, lambda) 연산을 상세히 설명합니다.',
    excerpt: '자료구조(Data Structure)의 필요성과 추상 자료형(ADT), 파이썬의 클래스 및 객체 모델, 리스트 연산의 성능 분석과 함수형 연산(filter, map) 기초를 다룹니다.',
    content_md: `**자료구조(Data Structure)**란 대용량의 데이터를 효율적으로 저장, 조직, 관리하기 위한 컴퓨터 과학의 핵심 기법입니다. 적절한 자료구조 선택은 알고리즘의 실행 속도와 메모리 사용량을 획기적으로 개선합니다.
...
`
  ...
  {
    order_index: 2,
    slug: 'ch02-linear-linked-lists',
    title: '선형 자료구조 - 단일·이중·원형 연결 리스트의 구조와 파이썬 구현',
    seo_title: '연결 리스트(Linked List) 완전 가이드 - 단일·이중·원형 연결 리스트 파이썬 구현',
    seo_description: '배열 대 연결 리스트 비교, 단일 연결 리스트(Singly), 이중 연결 리스트(Doubly), 원형 연결 리스트(Circular)의 노드 구조, 삽입, 삭제 및 순회 구현 코드를 제공합니다.',
    excerpt: '메모리의 비연속적 공간을 포인터로 엮어 관리하는 연결 리스트(Linked List)의 3가지 유형(단일, 이중, 원형)과 완벽한 파이썬 연산 코드를 배웁니다.',
    content_md: `**연결 리스트(Linked List)**는 데이터 필드와 포인터(Link) 필드를 가진 **노드(Node)**들이 동적으로 연결된 선형 자료구조입니다.
...
| **탐색 방향** | 단방향 (앞 $\rightarrow$ 뒤) | **양방향** | 순환 지속 탐색 가능 |
...
`
  ...
  {
    order_index: 3,
    slug: 'ch03-stacks-queues-and-deques',
    title: '스택(Stack), 큐(Queue), 덱(Deque)의 파이썬 구현 및 응용',
    seo_title: '스택, 큐, 덱(Deque) 자료구조 - 배열 vs 연결 리스트 구현 및 파이썬 활용',
    seo_description: 'LIFO 스택과 FIFO 큐, 양방향 덱(Deque)의 개념과 파이썬 구현(리스트 기반 vs 연결 리스트 기반), 괄호 쌍 검사, 원형 큐 동작 메커니즘을 학습합니다.',
    excerpt: '선형 자료구조의 3대장인 스택, 큐, 덱의 차이점, 파이썬 리스트 대 연결 리스트 구현 성능 비교, 실전 활용 사례를 심도있게 파악합니다.',
    content_md: `**스택(Stack)**, **큐(Queue)**, **덱(Deque)**은 데이터 입출력 방향에 제약을 부여하여 특정 문제 상황을 효율적으로 해결하게 돕는 선형 자료구조입니다.
...
`
  ...
  {
    order_index: 4,
    slug: 'ch04-binary-trees-and-heaps',
    title: '이진 트리(Binary Tree) 순회와 이진 힙(Binary Heap) 메커니즘',
    seo_title: '이진 트리와 이진 힙(Heap) 총정리 - 트리의 순회 및 힙 연산 파이썬 코드',
    seo_description: '이진 트리의 계층 구조, 전위/중위/후위/레벨 순회, 우선순위 큐를 위한 이진 힙(최소 힙, 최대 힙)의 배열 저장 방식과 삽입/삭제(Up-heap, Down-heap)를 살펴봅니다.',
    excerpt: '비선형 트리의 대표격인 이진 트리 순회 4가지 방식과, 우선순위 큐 구현의 핵심인 이진 힙(Heap)의 상향/하향 이동 메커니즘을 파이썬 코드와 분석합니다.',
    content_md: `**이진 트리(Binary Tree)**는 각 노드가 최대 2개의 자식 노드를 가지는 계층적 비선형 자료구조입니다. **이진 힙(Binary Heap)**은 완전 이진 트리 구조를 활용해 최댓값이나 최솟값을 $O(\log N)$에 찾는 최적 자료구조입니다.
...
`
  ...
  {
    order_index: 5,
    slug: 'ch05-search-trees-bst-and-avl',
    title: '탐색 트리 - 이진 탐색 트리(BST)와 자가 균형 AVL 트리 회전 연산',
    seo_title: '탐색 트리 완벽 이해 - 이진 탐색 트리(BST) 연산과 자가 균형 AVL 트리 회전',
    seo_description: 'BST의 탐색·삽입·삭제 동작과 편향 트리 문제, 높이 균형 조건(|BF| <= 1)을 유지하기 위한 AVL 트리의 4가지 회전(LL, RR, LR, RL) 연산을 코드와 함께 다룹니다.',
    excerpt: '이진 탐색 트리(BST)의 핵심 탐색·삭제 연산 및 트리가 한쪽으로 쏠리는 편향 문제를 자가 균형 회전 연산(LL, RR, LR, RL)으로 해결하는 AVL 트리를 다룹니다.',
    content_md: `**이진 탐색 트리(BST)**는 탐색 연산을 $O(\log N)$에 수행하도록 돕는 대표적 트리 구조이지만, 데이터 삽입 순서에 따라 $O(N)$ 편향 트리가 될 수 있습니다. 이를 방지하는 대표적 자가 균형 트리가 **AVL 트리**입니다.
...
`
  ...
  {
    order_index: 6,
    slug: 'ch06-hash-tables-and-collisions',
    title: '해시 테이블(Hash Table) 메커니즘과 충돌 해결 기법',
    seo_title: '해시 테이블과 충돌 해결 기법 - 선형/이차/재해싱 조사법 및 체이닝',
    seo_description: '키-값 매핑 해시 함수, 해시 충돌(Collision) 해결을 위한 개방 주소법(선형 조사, 이차 조사, 랜덤 조사/재해싱)과 체이닝(Chaining) 방식의 원리 및 구현을 설명합니다.',
    excerpt: '키(Key)를 주소로 직접 변환하여 평균 O(1) 탐색을 보장하는 해시 테이블과, 충돌 발생 시 개방 주소법과 체이닝 방식으로 해결하는 파이썬 코드를 살펴봅니다.',
    content_md: `**해시 테이블(Hash Table)**은 임의의 길이인 키(Key)를 **해시 함수(Hash Function)**를 거쳐 고정 크기의 테이블 인덱스로 변환해 데이터를 저장하는 $O(1)$ 자료구조입니다.
...
`
  ...
  {
    order_index: 7,
    slug: 'ch07-sorting-algorithms-in-python',
    title: '주요 정렬 알고리즘 (선택, 삽입, 쉘, 힙, 병합, 퀵, 기수 정렬) 분석',
    seo_title: '8대 정렬 알고리즘 파이썬 구현과 성능 비교 - 선택, 삽입, 쉘, 힙, 병합, 퀵, 기수 정렬',
    seo_description: '기본 정렬(선택, 삽입), 개선된 정렬(쉘, 힙, 병합, 퀵), 비비교 정렬(LSD 기수 정렬)의 동작 과정, 시간 및 공간 복잡도, 정렬 안정성을 파이썬 실습 코드와 비교 분석합니다.',
    excerpt: '8가지 주요 정렬 알고리즘의 동작 메커니즘과 파이썬 구현 코드, 시간/공간 복잡도, 그리고 파이썬 Timsort의 기초 이론까지 정밀하게 다룹니다.',
    content_md: `**정렬(Sorting)** 알고리즘은 데이터를 정해진 기준(오름차순/내림차순)으로 재배열하는 알고리즘으로, 탐색 연산 성능 최적화의 필수 전제조건입니다.
...
`
  ...
  {
    order_index: 8,
    slug: 'ch08-graph-structures-and-algorithms',
    title: '그래프 표현, DFS/BFS, 위상 정렬, MST, 최단경로 알고리즘',
    seo_title: '그래프 자료구조와 알고리즘 총정리 - DFS, BFS, 위상 정렬, Kruskal, Prim, Dijkstra, Floyd',
    seo_description: '인접 리스트/행렬 표현, 깊이/너비 우선 탐색(DFS/BFS), 연결 성분, DAG 위상 정렬, 최소 신장 트리(크루스칼, 프림), 최단 경로(다익스트라, 플로이드-워셜)를 완성형 코드와 분석합니다.',
    excerpt: '정점과 간선으로 이루어진 그래프의 기초부터 탐색(DFS/BFS), 위상 정렬, 최소 신장 트리(MST), 그리고 최단 경로 알고리즘까지 핵심 그래프 이론을 망라합니다.',
    content_md: `**그래프(Graph)**는 현실 세계의 도로망, 사회관계망(SNS), 인터넷 네트워크 등 객체 간의 유기적 관계를 정점(Vertex)과 간선(Edge)으로 모형화한 자료구조입니다.
...
| :--- | :--- | :--- |
| **목적** | 단일 출발점 $\rightarrow$ 모든 정점 최단 거리 | **모든 정점 쌍 간의 최단 거리** |
...
let sqlContent = `-- Seed Data Structure Lessons in D1 DB\n`;
...
fs.writeFileSync('scratch/seed_data_structure_lessons.sql', sqlContent, 'utf-8');
console.log('Successfully generated scratch/seed_data_structure_lessons.sql!');
