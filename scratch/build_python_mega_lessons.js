...
const lessons = [
  {
    ...
    seo_title: '파이썬 공식 가이드 - CPython 인터프리터, Zen of Python, IDLE & VS Code',
...
## 2. match-case 패턴 매칭 & zip 실습

\`\`\`python
# 1. 파이썬 3.10+ match-case 구조적 패턴 매칭
...
A. for 루프가 \`break\` 문을 만나 중단되지 않고 **정상적으로 처음부터 끝까지 모두 완주했을 때** 마지막에 \`else\` 블록이 1회 실행됩니다.
`
  },
  {
    order_index: 4,
...
A. \`close()\`를 하지 않으면 메모리 버퍼에 남아있는 데이터가 디스크에 즉시 기록되지 않거나, 파일 락 자원이 유지되어 다른 프로세스가 파일에 접근할 수 없게 됩니다. \`with\` 문을 쓰면 자동 마감됩니다.
`
  },
  {
    order_index: 8,
...
- **Iterator**: \`__next__()\` 메소드를 호출할 때마다 다음 요소를 하나씩 꺼내주는 순회 객체입니다.
...
- **datetime / timedelta**: 날짜와 시간을 생성하고 날짜 간의 차이를 계산하는 시간 처리 모듈입니다.
- **json (JSON Serialization)**: 파이썬 객체를 JSON 텍스트로 변환(\`dumps()\`)하거나 파싱(\`loads()\`)하는 직렬화 모듈입니다.
- **re (Regular Expression)**: 텍스트 내에서 특정 규칙을 가진 문자열 패턴을 검색/추출/치환하는 정규 표현식 모듈입니다.
...
print(f"이메일 유효성 검사: {bool(re.match(email_pattern, sample_email))}")
...
A. \`dumps()\`는 파이썬 객체를 JSON 형식의 **문자열(String)**로 반환하고, \`dump()\`는 바로 **파일 객체(File Stream)**에 JSON 데이터를 씁니다.
`
  },
  {
    order_index: 13,
...
chunks.forEach((chunk, index) => {
  let sqlContent = `-- Full Python Mega Lessons Part ${index + 1}\n`;
...
`;
  ...
  fs.writeFileSync(`scratch/seed_python_mega_p${index + 1}.sql`, sqlContent, 'utf-8');
});

console.log('Successfully generated 5 mega SQL chunk files for Python!');
